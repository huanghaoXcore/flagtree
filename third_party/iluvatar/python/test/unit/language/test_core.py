...
@pytest.mark.parametrize("dtype_x, expr, x",
                         [(dtype_x, expr, x)
                          for dtype_x in (["float32", "float64"] if not is_corex() else ["float32"])
...
@pytest.mark.interpreter
@pytest.mark.parametrize("dtype", [dtype for dtype in (["float32", "float64"] if not is_corex() else ["float32"])])
...
# -------------
@pytest.mark.interpreter
@pytest.mark.parametrize("expr_prec, expr_ref", [('tl.math.sqrt_rn(x)', 'tl.math.sqrt(x)'),
                                                 ('tl.math.div_rn(x,y)', 'x / y')])
...
@pytest.mark.parametrize("dtype_x, dtype_z, bitcast, size",
                         ...
                         (([(dtype_x, dtype_z, False, size)
                           ...
                           + [(dtype_x, dtype_z, False, size)
                              ...
                              for size in [1024, 32]]) if torch.__version__ >= "2.1" and not is_corex() else []))
@pytest.mark.parametrize("num_ctas", num_ctas_list)
...
# , (32, 64), (64, 128)]
if is_corex() or is_cuda() and 'V100' in torch.cuda.get_device_name(0):
...
# TODO: beflow 2 configs can not pass somehow.
#failed_config = [('linear_recurrence', 'float32', (4, 32), 0, True, 1),
#                 ('linear_recurrence', 'int32', (4, 32), 0, True, 1)]
...
def test_scan_layouts(M, N, src_layout, axis, device):
    ...
    tt.func public @kernel_0d1d(%arg0: !tt.ptr<i32> {{tt.divisibility = 16 : i32}}, %arg1: !tt.ptr<i32> {{tt.divisibility = 16 : i32}}) {{
      ...
      %1 = tt.expand_dims %0 {{axis = 1 : i32}} : tensor<{M}xi32, #triton_gpu.slice<{{dim = 1, noWarpReduce=false, parent = #blocked}}>> -> tensor<{M}x1xi32, #blocked>
      ...
      %5 = tt.make_range {{end = {N} : i32, start = 0 : i32}} : tensor<{N}xi32, #triton_gpu.slice<{{dim = 0, noWarpReduce=false, parent = #blocked}}>>
      %6 = tt.expand_dims %5 {{axis = 0 : i32}} : tensor<{N}xi32, #triton_gpu.slice<{{dim = 0, noWarpReduce=false, parent = #blocked}}>> -> tensor<1x{N}xi32, #blocked>
      ...
      %10 = tt.load %9 : tensor<{M}x{N}xi32, #blocked>
...
def test_reduce_layouts(M, N, src_layout, axis, epilogue_kind, dtype_str, reduce_op, device):
    ...
    epilogue = {
        ...
        %16 = {GPU_DIALECT}.convert_layout %13 : tensor<{rdims_1d}x{ty}, #{GPU_DIALECT}.slice<{{dim = {axis}, noWarpReduce=false, parent = #src}}>> -> tensor<{rdims_1d}x{ty}, #{GPU_DIALECT}.slice<{{dim = {axis}, noWarpReduce=false, parent = #blocked}}>>
        %17 = tt.expand_dims %16 {{axis = {axis} : i32}} : tensor<{rdims_1d}x{ty}, #{GPU_DIALECT}.slice<{{dim = {axis}, noWarpReduce=false, parent = #blocked}}>> -> tensor<{rdims_2d}x{ty}, #blocked>
    ...
    """, "reduce2d":
        ...
        }}) {{axis = 0 : i32, noWarpReduce=false}} : (tensor<{rdims_1d}x{ty}, #{GPU_DIALECT}.slice<{{dim = {axis}, noWarpReduce=false, parent = #src}}>>) -> {ty}
    ...
    """, "expand_reduce2d":
        f"""
        %14 = tt.expand_dims %13 {{axis = {axis} : i32}} : tensor<{rdims_1d}x{ty}, #{GPU_DIALECT}.slice<{{dim = {axis}, noWarpReduce=false, parent = #src}}>> -> tensor<{expanded_shape}x{ty}, #src>
        %15 = "tt.reduce"(%14) ({{
        ^bb0(%arg3: {ty}, %arg4: {ty}):
          %17 = {arith_op} %arg3, %arg4 : {ty}
          tt.reduce.return %17 : {ty}
        }}) {{axis = {other_axis} : i32, noWarpReduce=false}} : (tensor<{expanded_shape}x{ty}, #src>) -> (tensor<1x{ty}, #{GPU_DIALECT}.slice<{{dim = {other_axis}, noWarpReduce=false, parent = #src}}>>)
        %16 = triton_gpu.convert_layout %15 : tensor<1x{ty}, #{GPU_DIALECT}.slice<{{dim = {other_axis}, noWarpReduce=false, parent = #src}}>> -> tensor<1x{ty}, #one_d_layout>
    ...
    tt.func public @kernel_0d1d2c3d4c(%arg0: !tt.ptr<{ty}> {{tt.divisibility = 16 : i32}}, %arg1: i32 {{tt.divisibility = 16 : i32}}, %arg2: !tt.ptr<{ty}> {{tt.divisibility = 16 : i32}}) {{
        ...
        %6 = tt.make_range {{end = {N} : i32, start = 0 : i32}} : tensor<{N}xi32, #{GPU_DIALECT}.slice<{{dim = 0, noWarpReduce=false, parent = #blocked}}>>
        %7 = tt.expand_dims %6 {{axis = 0 : i32}} : tensor<{N}xi32, #{GPU_DIALECT}.slice<{{dim = 0, noWarpReduce=false, parent = #blocked}}>> -> tensor<1x{N}xi32, #blocked>
        ...
        %11 = tt.load %10 : tensor<{M}x{N}x{ty}, #blocked>
        %12 = {GPU_DIALECT}.convert_layout %11 : tensor<{M}x{N}x{ty}, #blocked> -> tensor<{M}x{N}x{ty}, #src>
        %13 = "tt.reduce"(%12) ({{
        ^bb0(%arg3: {ty}, %arg4: {ty}):
          %17 = {arith_op} %arg3, %arg4 : {ty}
          tt.reduce.return %17 : {ty}
        }}) {{axis = {axis} : i32, noWarpReduce=false}} : (tensor<{M}x{N}x{ty}, #src>) -> tensor<{rdims_1d}x{ty}, #{GPU_DIALECT}.slice<{{dim = {axis}, noWarpReduce=false, parent = #src}}>>
    """ + epilogue
...
def test_store_op(M, src_layout, device):
    ...
    module attributes {{"triton_gpu.dot.num-stages" = 1 : i32, "{GPU_DIALECT}.num-warps" = 4 : i32, {GPU_DIALECT}.target = "cuda:{cc}", "{GPU_DIALECT}.num-ctas" = 1 : i32, "{GPU_DIALECT}.threads-per-warp" = {THREADS_PER_WARP} : i32}} {{
        tt.func public @kernel(%arg0: !tt.ptr<f32> {{tt.divisibility = 16 : i32}}, %arg1: !tt.ptr<f32> {{tt.divisibility = 16 : i32}}) {{
            ...
            %2 = tt.addptr %1, %0 : tensor<{M}x!tt.ptr<f32>, #{GPU_DIALECT}.slice<{{dim = 1, noWarpReduce=false, parent = #src}}>>, tensor<{M}xi32, #{GPU_DIALECT}.slice<{{dim = 1, noWarpReduce=false, parent = #src}}>>
            %3 = tt.load %2 : tensor<{M}xf32, #{GPU_DIALECT}.slice<{{dim = 1, noWarpReduce=false, parent = #src}}>>
            %4 = tt.expand_dims %3 {{axis = 1 : i32}} : tensor<{M}xf32, #{GPU_DIALECT}.slice<{{dim = 1, noWarpReduce=false, parent = #src}}>> -> tensor<{M}x1xf32, #src>
            %5 = tt.make_range {{end = {M} : i32, start = 0 : i32}} : tensor<{M}xi32, #{GPU_DIALECT}.slice<{{dim = 1, noWarpReduce=false, parent = #src}}>>
            %6 = tt.expand_dims %5 {{axis = 1 : i32}} : tensor<{M}xi32, #{GPU_DIALECT}.slice<{{dim = 1, noWarpReduce=false, parent = #src}}>> -> tensor<{M}x1xi32, #src>
...
def test_convert1d(M, src_layout, dst_layout, src_dim, dst_dim, device):
    ...
    module attributes {{"triton_gpu.dot.num-stages" = 1 : i32, "{GPU_DIALECT}.num-warps" = 4 : i32, {GPU_DIALECT}.target = "cuda:{cc}", "triton_gpu.num-ctas" = 1 : i32, "triton_gpu.threads-per-warp" = {THREADS_PER_WARP} : i32}} {{
        tt.func public @kernel(%arg0: !tt.ptr<i32> {{tt.divisibility = 16 : i32}}, %arg1: !tt.ptr<i32> {{tt.divisibility = 16 : i32}}) {{
            ...
            %2 = tt.addptr %0, %1 : tensor<{M}x!tt.ptr<i32>, #{GPU_DIALECT}.slice<{{dim = {src_dim}, noWarpReduce=false, parent = #src}}>>, tensor<{M}xi32, #{GPU_DIALECT}.slice<{{dim = {src_dim}, noWarpReduce=false, parent = #src}}>>
            %3 = tt.load %2 : tensor<{M}xi32, #{GPU_DIALECT}.slice<{{dim = {src_dim}, noWarpReduce=false, parent = #src}}>>
            %4 = tt.splat %arg1 : !tt.ptr<i32> -> tensor<{M}x!tt.ptr<i32>, #{GPU_DIALECT}.slice<{{dim = {dst_dim}, noWarpReduce=false, parent = #dst}}>>
            %5 = tt.make_range {{end = {M} : i32, start = 0 : i32}} : tensor<{M}xi32, #{GPU_DIALECT}.slice<{{dim = {dst_dim}, noWarpReduce=false, parent = #dst}}>>
            %6 = tt.addptr %4, %5 : tensor<{M}x!tt.ptr<i32>, #{GPU_DIALECT}.slice<{{dim = {dst_dim}, noWarpReduce=false, parent = #dst}}>>, tensor<{M}xi32, #{GPU_DIALECT}.slice<{{dim = {dst_dim}, noWarpReduce=false, parent = #dst}}>>
            %7 = {GPU_DIALECT}.convert_layout %3 : tensor<{M}xi32, #{GPU_DIALECT}.slice<{{dim = {src_dim}, noWarpReduce=false, parent = #src}}>> -> tensor<{M}xi32, #{GPU_DIALECT}.slice<{{dim = {dst_dim}, noWarpReduce=false, parent = #dst}}>>
            tt.store %6, %7 : tensor<{M}x!tt.ptr<i32>, #{GPU_DIALECT}.slice<{{dim = {dst_dim}, noWarpReduce=false, parent = #dst}}>>
            tt.return
        }}
    }}
    """
...
def test_chain_reduce(M, N, src_layout, op, device, first_axis):
    ...
    tt.func public @sum_kernel_0d1d(%arg0: !tt.ptr<i32> {{tt.divisibility = 16 : i32}}, %arg1: !tt.ptr<i32> {{tt.divisibility = 16 : i32}}) {{
        ...
        %0 = tt.make_range {{end = {M} : i32, start = 0 : i32}} : tensor<{M}xi32, #{GPU_DIALECT}.slice<{{dim = 1, noWarpReduce=false, parent = #src}}>>
        %1 = tt.expand_dims %0 {{axis = 1 : i32}} : tensor<{M}xi32, #{GPU_DIALECT}.slice<{{dim = 1, noWarpReduce=false, parent = #src}}>> -> tensor<{M}x1xi32, #src>
        %2 = arith.muli %1, %cst : tensor<{M}x1xi32, #src>
        %3 = tt.make_range {{end = {N} : i32, start = 0 : i32}} : tensor<{N}xi32, #{GPU_DIALECT}.slice<{{dim = 0, noWarpReduce=false, parent = #src}}>>
        %4 = tt.expand_dims %3 {{axis = 0 : i32}} : tensor<{N}xi32, #{GPU_DIALECT}.slice<{{dim = 0, noWarpReduce=false, parent = #src}}>> -> tensor<1x{N}xi32, #src>
        ...
        %10 = tt.load %9 : tensor<{M}x{N}xi32, #src>
        %11 = "tt.reduce"(%10) ({{
        ^bb0(%arg2: i32, %arg3: i32):
        {op_str}
        }}) {{axis = {first_axis} : i32, noWarpReduce=false}} : (tensor<{M}x{N}xi32, #src>) -> tensor<{M if first_axis == 1 else N}xi32, #{GPU_DIALECT}.slice<{{dim = {first_axis}, noWarpReduce=false, parent = #src}}>>
        %12 = "tt.reduce"(%11) ({{
        ^bb0(%arg2: i32, %arg3: i32):
        {op_str}
        }}) {{axis = 0 : i32, noWarpReduce=false}} : (tensor<{M if first_axis == 1 else N}xi32, #{GPU_DIALECT}.slice<{{dim = {first_axis}, noWarpReduce=false, parent = #src}}>>) -> i32
...
def test_dot3d(B, num_warps, M, N, K, in_dtype_str, out_dtype_str, device):
    if is_corex():
        pytest.skip("iluvatar devices does not support dot3d now")
    elif is_hip():
...
def test_globaltimer(device):
    if is_hip() or is_corex():
        pytest.skip("test_globaltimer is not supported in HIP and COREX")
...
def test_smid(device):
    if is_hip() or is_corex():
        pytest.skip("test_smid is not supported in HIP and COREX")
    check_cuda_or_hip(device)
    ...
    if is_corex():
        ir = layouts + f"""
            ...
            tt.func public @kernel_0d1d(%arg0: !tt.ptr<f16> {{tt.divisibility = 16 : i32}}, %arg1: !tt.ptr<f16> {{tt.divisibility = 16 : i32}}) {{
                ...
                %1 = tt.make_range {{end = {N} : i32, start = 0 : i32}} : tensor<{N}xi32, #triton_gpu.slice<{{dim = 0, noWarpReduce=false, parent = #src}}>>
                %2 = tt.splat %arg0 : !tt.ptr<f16> -> tensor<{M}x{N}x!tt.ptr<f16>, #src>
                %4 = tt.expand_dims %0 {{axis = 1 : i32}} : tensor<{M}xi32, #triton_gpu.slice<{{dim = 1, noWarpReduce=false, parent = #src}}>> -> tensor<{M}x1xi32, #src>
                %5 = arith.muli %4, %cst : tensor<{M}x1xi32, #src>
                %6 = tt.expand_dims %1 {{axis = 0 : i32}} : tensor<{N}xi32, #triton_gpu.slice<{{dim = 0, noWarpReduce=false, parent = #src}}>> -> tensor<1x{N}xi32, #src>
                ...
                %11 = tt.load %10 : tensor<{M}x{N}xf16, #src>
...
def test_fp8_dot_acc(in_type_str, low_precision_acc, device):
    ...
    if is_cuda():
        cc = torch.cuda.get_device_capability()
        if is_corex():
            pytest.skip('test_fp8_dot_acc for iluvatar does not support.')
        else:
