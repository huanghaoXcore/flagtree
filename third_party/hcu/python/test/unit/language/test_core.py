...
class MfmaLayout:
    ...
    def __str__(self):
        return f"#{GPU_DIALECT}.hcu_mfma<{{versionMajor={self.version[0]}, versionMinor={self.version[1]}, warpsPerCTA = {self.warps_per_cta}, instrShape={self.instr_shape}, isTransposed = {str(self.is_transposed).lower()}}}>"
...
class WmmaLayout:
    ...
    def __str__(self):
        return f"#{GPU_DIALECT}.hcu_wmma<{{version = {self.version}, warpsPerCTA = {self.warps_per_cta}}}>"
...
def test_optimize_thread_locality(op, BLOCK_N, N, num_pid_n, device):
    ...
    def kernel(X, Y, N, BLOCK_M: tl.constexpr, BLOCK_N: tl.constexpr, NUM_PID_N: tl.constexpr):
        ...
        # the following segfaults HCU backend following #3492
...
@pytest.mark.parametrize("epilogue_kind", ['reduce1d'])
...
def test_load_cache_modifier(cache, device):
    ...
    if is_hip():
        ...
        if 'gfx94' not in target_arch:
            return
        hcugcn = pgm.asm['amdgcn']
        cg_cache_modifier_str = 'nt'
        cv_cache_modifier_str = 'sc0 sc1'
        global_load_line = [line for line in hcugcn.splitlines() if "global_load" in line]
        flat_load_line = [line for line in hcugcn.splitlines() if "flat_load" in line]
...
def test_store_cache_modifier(cache, device):
    ...
    if is_hip():
        ...
        global_store_line = [line for line in hcugcn.splitlines() if "global_store" in line]
