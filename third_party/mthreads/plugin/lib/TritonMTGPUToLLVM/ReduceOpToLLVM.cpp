...
public:
  ...
  matchAndRewrite(triton::ReduceOp op, OpAdaptor adaptor,
    ...
    auto layout = helper.getSrcLayout();
...
private:
  ...
  void warpReduceNormal(ConversionPatternRewriter &rewriter, Location loc,
  ...
  void warpReduceMMA(ConversionPatternRewriter &rewriter, Location loc,
                     SmallVector<Value> &acc, triton::ReduceOp op) const {
    RankedTensorType operandType = op.getInputTypes()[0];
    auto layout = operandType.getEncoding();

    SmallVector<unsigned> reduceStep;
    if (op.getAxis() == 0) {
      reduceStep = {64, 16, 8};
    } else if (op.getAxis() == 1) {
      reduceStep = {32, 4, 2, 1};
    }

    for (int index = 0; index < reduceStep.size(); index++) {
      SmallVector<Value> shfl(acc.size());
      for (unsigned i = 0; i < acc.size(); ++i) {
        shfl[i] =
            targetInfo.shuffleXor(rewriter, loc, acc[i], reduceStep[index]);
      }
      accumulate(rewriter, op.getCombineOp(), acc, shfl, false);
    }
    return;
  ...
  reduceWithinWarps(ReduceOpHelper &helper,
    ...
    for (auto it : accs) {
      ...
      warpReduceMMA(rewriter, op.getLoc(), acc, op);
    }
  }
  ...
  void storeWarpReduceToSharedMemory(
    ...
    Value warpId = udiv(threadId, i32_val(128));
    Value warpRemainder = urem(threadId, warpSize);
    Value mpeId = udiv(warpRemainder, i32_val(32));
    Value laneId = urem(warpRemainder, i32_val(32));
    ...
    SmallVector<Value> multiDimMpeId =
        delinearize(rewriter, loc, mpeId, {2, 2}, order);
    SmallVector<Value> multiDimLaneId =
        delinearize(rewriter, loc, laneId, {4, 8}, order);
    Value mpeIdAxis = multiDimMpeId[axis];
    Value laneIdAxis = multiDimLaneId[axis];
    Value zero = i32_val(0);
    Value laneZero = icmp_eq(laneIdAxis, zero);
    Value mpeZero = icmp_eq(mpeIdAxis, zero);
    Value isThreadNeeded = and_(laneZero, mpeZero);
    ...
    for (auto it : accs) {
      ...
      for (unsigned i = 0; i < op.getNumOperands(); ++i) {
        ...
        targetInfo.storeShared(rewriter, loc, writePtr, acc[i], isThreadNeeded);
      }
    }
  }
  ...
  void accumulatePartialReductions(ReduceOpHelper &helper,
    ...
    for (unsigned round = 0; round < elemsPerThread; ++round) {
      ...
      warpReduceNormal(rewriter, loc, acc, op, sizeInterWarps, 1);
...
void mlir::triton::MUSA::populateReduceOpToLLVMPatterns(
