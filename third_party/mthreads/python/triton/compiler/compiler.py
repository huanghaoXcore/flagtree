...
def convert_type_repr(x):
    ...
    tma = re.search(r'tt.mt_tma_desc = 1', x)
    if tma is not None:
        return 'mtTmaDesc'
    x = re.sub(r' {[^}]+}', '', x)
...
def compile(src, target=None, options=None):
    ...
    replace_ir = os.environ.get("MUSA_REPLACE_IR", "0") == "1"
    if not always_compile and metadata_path is not None and not replace_ir:
        # cache hit!
    ...
    for ext, compile_ir in list(stages.items())[first_stage:]:
        ...
        if ext == "mubin":
            # next_module is a tuple of (muasm, path_to_mubin). Copy the binary into cache
            # so cached kernels never reference temporary files that might be removed later.
            mubin_path = next_module[1]
            with open(mubin_path, "rb") as f:
                metadata_group[ir_filename] = fn_cache_manager.put(f.read(), ir_filename)
        else:
