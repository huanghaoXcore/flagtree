...
def computation_type_impl(a_ty: tl.dtype, a_is_scalar: bool, b_ty: tl.dtype, b_is_scalar: bool,
    ...
    if a_is_scalar != b_is_scalar:
        ...
        scalar_kind = scalar_ty.kind().value
        tensor_kind = tensor_ty.kind().value
        if scalar_kind < tensor_kind:
            if div_or_mod and (tensor_ty in (tl.float16, tl.bfloat16)):
                return tl.float32
            return tensor_ty
        # For same-kind scalars/tensors (e.g. float scalar + bf16 tensor), fall
        # through to the general promotion rules so higher precision scalars can
        # keep the computation in fp32 and match pre-3.2 behaviour.
...
def assert_dtypes_valid(lhs_dtype: tl.dtype, rhs_dtype: tl.dtype, options) -> None:
    supported_fp8 = set(getattr(options, "supported_fp8_dtypes", ()))
    deprecated_fp8 = set(getattr(options, "deprecated_fp8_dtypes", ()))

    if lhs_dtype.is_fp8() or rhs_dtype.is_fp8():
        assert lhs_dtype.is_fp8() and rhs_dtype.is_fp8(), \
            f"FP8 operands must appear in pairs. Got {lhs_dtype} and {rhs_dtype}"

        for dtype in (lhs_dtype, rhs_dtype):
            dtype_name = str(dtype)
            if supported_fp8 and dtype_name not in supported_fp8 and dtype_name not in deprecated_fp8:
                raise ValueError(f"{dtype_name} is not supported for dot on this target. "
                                 f"Supported fp8 dtypes: {sorted(supported_fp8)}")
            if dtype_name in deprecated_fp8:
                warnings.warn(
                    f"{dtype_name} is deprecated for dot on this target "
    ...
    allowed = (tl.int8, tl.uint8, tl.float16, tl.bfloat16, tl.float32)
    assert lhs_dtype in allowed, f"Unsupported lhs dtype {lhs_dtype}"
    assert rhs_dtype in allowed, f"Unsupported rhs dtype {rhs_dtype}"
