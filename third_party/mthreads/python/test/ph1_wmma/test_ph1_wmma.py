...
BLOCK_WARP_CONFIG = [
    (torch.float16, 8, 16, 16, 1),
    (torch.float16, 16, 8, 8, 1),
    (torch.float16, 16, 8, 16, 1),
    (torch.float16, 16, 16, 16, 1),
    (torch.float16, 16, 16, 32, 1),
    (torch.bfloat16, 8, 16, 16, 1),
    (torch.bfloat16, 16, 8, 8, 1),
    (torch.bfloat16, 16, 8, 16, 1),
    (torch.bfloat16, 16, 16, 16, 1),
    (torch.bfloat16, 16, 16, 32, 1),
    (torch.float8_e4m3fn, 8, 16, 16, 1),
    (torch.float8_e4m3fn, 16, 8, 16, 1),
    (torch.float8_e4m3fn, 16, 16, 16, 1),
    (torch.float8_e4m3fn, 16, 16, 32, 1),
    (torch.float8_e4m3fn, 16, 16, 64, 1),
    (torch.float8_e5m2, 8, 16, 16, 1),
    (torch.float8_e5m2, 16, 8, 16, 1),
    (torch.float8_e5m2, 16, 16, 16, 1),
    (torch.float8_e5m2, 16, 16, 32, 1),
    (torch.float8_e5m2, 16, 16, 64, 1),
]
CONFIG = [(M, N, K, dtype, block_m, block_n, block_k, num_warps)
          for M, N, K in MNK
          for dtype, block_m, block_n, block_k, num_warps in BLOCK_WARP_CONFIG]


@pytest.mark.parametrize("M,N,K,dtype,block_m,block_n,block_k,num_warps", CONFIG)
