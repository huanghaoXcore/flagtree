...
def matmul_kernel_tma_k16(
    ...
    for _ in range(0, tl.cdiv(K, BLOCK_SIZE_K)):
        ...
        offs_k += BLOCK_SIZE_K
    tl._experimental_descriptor_store(c_desc_ptr, accumulator.to(tl.float16), [offs_am, offs_bn])
...
def test_sqmma_k16_tiles(M, N, K, BLOCK_M, BLOCK_N, BLOCK_K):
    device = "musa"
    torch.manual_seed(0)
    ...
    matmul_kernel_tma_k16[(triton.cdiv(M, BLOCK_M) * triton.cdiv(N, BLOCK_N), 1, 1)](
        desc_a,
        desc_b,
        desc_c,
        M,
