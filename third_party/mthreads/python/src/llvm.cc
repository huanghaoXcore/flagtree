...
static void rewriteZeroVectorBitcasts(mlir::ModuleOp mod) {
  SmallVector<mlir::LLVM::BitcastOp, 4> toErase;
  mlir::OpBuilder builder(mod.getContext());
  mod.walk([&](mlir::LLVM::BitcastOp bitcast) {
    auto vecTy =
        mlir::dyn_cast<mlir::VectorType>(bitcast.getResult().getType());
    if (!vecTy)
      return;
    auto constOp = bitcast.getArg().getDefiningOp<mlir::LLVM::ConstantOp>();
    if (!constOp)
      return;
    ...
    auto denseAttr = mlir::SplatElementsAttr::get(vecTy, floatAttr);
    auto zeroVec = builder.create<mlir::LLVM::ConstantOp>(bitcast.getLoc(),
                                                          vecTy, denseAttr);
    bitcast.getResult().replaceAllUsesWith(zeroVec);
    toErase.push_back(bitcast);
  });
  for (auto bitcast : toErase)
    bitcast.erase();
}
...
void init_triton_llvm(py::module &&m) {
  ...
  m.def(
      ...
      [](llvm::Module *mod, const llvm::OptimizationLevel &opt,
        ...
        // FIXME: SLPVectorization generates some large vectors,
        // such as <64 * float>, which mtcc cannot currently handle.(SW-47321)
