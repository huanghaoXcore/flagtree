...
bool supportMMA(triton::DotOp op, int version) {
  ...
  if (version == 3) {
    ...
    if (op.getMaxNumImpreciseAcc() < 32 &&
      ...
      auto mmaEncoding = dstTy.getEncoding();
      auto mthreadEncoding = dyn_cast<MthreadsSqmmaEncodingAttr>(mmaEncoding);
      if (mthreadEncoding && mthreadEncoding.isPH1()) {
        assert(0 && "ph1 should support this feature");
      }
      return false;
    }
  }
