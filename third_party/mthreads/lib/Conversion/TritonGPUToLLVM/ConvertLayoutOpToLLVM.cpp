...
static void processReplica(
    Location loc, ConversionPatternRewriter &rewriter,
    const LLVMTypeConverter &typeConverter, const TargetInfoBase &targetInfo,
    bool stNotRd, RankedTensorType type, ArrayRef<unsigned> numCTAsEachRep,
  ...
  for (unsigned ctaId = 0; ctaId < accumNumCTAsEachRep; ++ctaId) {
    ...
    for (unsigned elemId = 0; elemId < accumSizePerThread; elemId += vec) {
      ...
      SmallVector<Value> multiDimOffsetWrapped =
          getWrappedMultiDimOffset(rewriter, loc, multiDimOffset, origRepShape,
...
static LogicalResult lowerDistributedToDistributedLegacy(
    ConvertLayoutOp op, ConvertLayoutOpAdaptor adaptor,
  ...
  for (unsigned repId = 0; repId < accumNumReplicates; ++repId) {
    ...
    processReplica(loc, rewriter, typeConverter, targetInfo, /*stNotRd*/ true,
                   srcTy, inNumCTAsEachRep, multiDimRepId, inVec,
                   paddedRepShape, origRepShape, outOrd, vals, smemBase);
    insertBarrier(rewriter, op);
    processReplica(loc, rewriter, typeConverter, targetInfo,
                   /*stNotRd*/ false, dstTy, outNumCTAsEachRep, multiDimRepId,
...
public:
  ...
  matchAndRewrite(ConvertLayoutOp op, OpAdaptor adaptor,
    ...
    if (isSupported(srcLayout, dstLayout)) {
      return lowerDistributedToDistributedLegacy(
          op, adaptor, rewriter, targetInfo, *getTypeConverter());
    }
    return failure();
  }
...
struct ConvertLayoutOpUsingLinearLayoutsConversion
  ...
  matchAndRewrite(ConvertLayoutOp op, OpAdaptor adaptor,
    ...
    bool needsShared = cvtNeedsSharedMemory(srcTy, dstTy);
    if (isa<BlockedEncodingAttr>(srcTy.getEncoding()) &&
        isa<BlockedEncodingAttr>(dstTy.getEncoding()) && needsShared) {
      return lowerDistributedToDistributedLegacy(
          op, adaptor, rewriter, targetInfo, *getTypeConverter());
    }
...
void mlir::triton::populateConvertLayoutOpToLLVMPatterns(
  ...
  // one left.
  if (targetInfo.supportsLinearLayouts())
