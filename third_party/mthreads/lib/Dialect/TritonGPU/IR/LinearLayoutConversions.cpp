...
LinearLayout QY2WMMAToABLinearLayout(ArrayRef<int64_t> shape,
                                     DotOperandEncodingAttr dot) {
  bool isA = dot.getOpIdx() == 0;
  auto mma = cast<MthreadsWmmaEncodingAttr>(dot.getParent());
  MLIRContext *ctx = mma.getContext();
  if (isA) {
    LinearLayout ctaLayout(
        {{S("register"), {}},
         ...
         {S("block"), {}}},
        {S("dim1"), S("dim0")});
    ctaLayout *= LinearLayout::identity1D(2, S("lane"), S("dim0"));
    ctaLayout *= LinearLayout::identity1D(2, S("lane"), S("dim1"));
    ctaLayout *= LinearLayout::identity1D(2, S("register"), S("dim0"));
    return combineCtaCgaWithShape(ctaLayout, mma.getCTALayout(), shape);
  } else {
    LinearLayout ctaLayout(
        {{S("register"), {}},
         ...
         {S("block"), {}}},
        {S("dim1"), S("dim0")});
    ctaLayout *= LinearLayout::identity1D(4, S("lane"), S("dim0"));
    ctaLayout *= LinearLayout::identity1D(2, S("register"), S("dim1"));
    return combineCtaCgaWithShape(ctaLayout, mma.getCTALayout(), shape);
  }
}

LinearLayout QY2WMMAToCLinearLayout(ArrayRef<int64_t> shape,
                                    MthreadsWmmaEncodingAttr mma) {
  MLIRContext *ctx = mma.getContext();
  ...
  ctaLayout *= LinearLayout::identity1D(2, S("lane"), S("dim0"));
  ctaLayout *= LinearLayout::identity1D(2, S("register"), S("dim1"));
  ctaLayout *= LinearLayout::identity1D(4, S("register"), S("dim0"));
  return combineCtaCgaWithShape(ctaLayout, mma.getCTALayout(), shape);
}

LinearLayout PH1WMMAToABLinearLayout(ArrayRef<int64_t> shape,
                                     DotOperandEncodingAttr dot) {
  bool isA = dot.getOpIdx() == 0;
  auto mma = cast<MthreadsWmmaEncodingAttr>(dot.getParent());
  unsigned INST_M = mma.getInstrShape()[0];
  unsigned INST_N = mma.getInstrShape()[1];
  unsigned INST_K = mma.getInstrShape()[2];
  auto warpsPerCTA = mma.getWarpsPerCTA();
  unsigned TILE_M = warpsPerCTA[0];
  unsigned TILE_N = warpsPerCTA[1];
  MLIRContext *ctx = mma.getContext();
  if (isA) {
    unsigned BLOCK_M = shape[0];
    unsigned BLOCK_K = shape[1];
    LinearLayout ctaLayout(
        {{S("register"), {}},
         ...
         {S("block"), {}}},
        {S("dim0"), S("dim1")});
    ctaLayout *= LinearLayout::identity1D(INST_K / 4, S("register"), S("dim1"));
    ctaLayout *= LinearLayout::identity1D(INST_M / 8, S("register"), S("dim0"));
    ctaLayout *=
        LinearLayout::identity1D(BLOCK_K / INST_K, S("register"), S("dim1"));
    ctaLayout *= LinearLayout::identity1D(TILE_M, S("warp"), S("dim0"));
    ctaLayout *= LinearLayout::identity1D(BLOCK_M / INST_M / TILE_M,
                                          S("register"), S("dim0"));
    return combineCtaCgaWithShape(ctaLayout, mma.getCTALayout(), shape);
  } else {
    unsigned BLOCK_K = shape[0];
    unsigned BLOCK_N = shape[1];
    LinearLayout ctaLayout(
        {{S("register"), {}},
         {S("lane"), {{1, 0}, {2, 0}, {0, 1}, {0, 2}, {0, 4}}},
         {S("warp"), {}},
         {S("block"), {}}},
        {S("dim0"), S("dim1")});
    ctaLayout *= LinearLayout::identity1D(INST_K / 4, S("register"), S("dim0"));
    ctaLayout *= LinearLayout::identity1D(INST_N / 8, S("register"), S("dim1"));
    ctaLayout *= LinearLayout::identity1D(TILE_N, S("warp"), S("dim1"));
    ctaLayout *= LinearLayout::identity1D(BLOCK_N / INST_N / TILE_N,
                                          S("register"), S("dim1"));
    ctaLayout *=
        LinearLayout::identity1D(BLOCK_K / INST_K, S("register"), S("dim0"));
    return combineCtaCgaWithShape(ctaLayout, mma.getCTALayout(), shape);
  }
}

LinearLayout PH1WMMAToCLinearLayout(ArrayRef<int64_t> shape,
                                    MthreadsWmmaEncodingAttr mma) {
  MLIRContext *ctx = mma.getContext();
  unsigned BLOCK_M = shape[0];
  unsigned BLOCK_N = shape[1];
  ...
  LinearLayout ctaLayout({{S("register"), {}},
                         ...
                         {S("dim0"), S("dim1")});
  ctaLayout *= LinearLayout::identity1D(INST_N / 8, S("register"), S("dim1"));
  ctaLayout *= LinearLayout::identity1D(INST_M / 4, S("register"), S("dim0"));
  ctaLayout *= LinearLayout::identity1D(BLOCK_N / INST_N / TILE_N,
                                        S("register"), S("dim1"));
  ctaLayout *= LinearLayout::identity1D(BLOCK_M / INST_M / TILE_M,
                                        S("register"), S("dim0"));
...
LinearLayout PH1SQMMAToCLinearLayout(ArrayRef<int64_t> shape,
                                     MthreadsSqmmaEncodingAttr mma) {
  int rank = shape.size();
  assert(mma.isPH1());
  assert(rank == 2);
...
std::optional<LinearLayout>
MthreadsWmmaEncodingAttr::toLinearLayout(ArrayRef<int64_t> shape) const {
  if (isPH1()) {
    return PH1WMMAToCLinearLayout(shape, *this);
  }
  // return QY2WMMAToCLinearLayout(shape, *this);
  // Disable WMMA linear layout so WMMA paths continue to use legacy lowering.
  return std::nullopt;
}

std::optional<LinearLayout>
MthreadsSqmmaEncodingAttr::toLinearLayout(ArrayRef<int64_t> shape) const {
  return PH1SQMMAToCLinearLayout(shape, *this);
}
...
SliceEncodingAttr::toLinearLayout(ArrayRef<int64_t> shape) const {
  ...
  if (!parentLL.has_value()) {
    if (mlir::isa<DotOperandEncodingAttr>(getParent()) ||
        mlir::isa<MthreadsWmmaEncodingAttr>(getParent()))
      return std::nullopt;
...
DotOperandEncodingAttr::toLinearLayout(ArrayRef<int64_t> shape) const {
  ...
  } else if (auto mma = mlir::dyn_cast<MthreadsWmmaEncodingAttr>(getParent())) {
    if (mma.isQY2()) {
      // return QY2WMMAToABLinearLayout(shape, *this);
      // Disable WMMA linear layout so WMMA paths continue to use legacy
      // lowering.
      return std::nullopt;
    } else if (mma.isPH1()) {
      return PH1WMMAToABLinearLayout(shape, *this);
    }
  }
  return std::nullopt;
}
...
static bool isSupportedStMatrixEncoding(Attribute encoding) {
#ifdef USE_MUSA
  if (auto mt = dyn_cast<MthreadsSqmmaEncodingAttr>(encoding))
    return mt.isPH1();
  return false;
#else
  if (auto mma = dyn_cast<NvidiaMmaEncodingAttr>(encoding))
    return mma.isHopper();
  if (auto mt = dyn_cast<MthreadsSqmmaEncodingAttr>(encoding))
    return mt.isPH1();
  return false;
...
static SmallVector<unsigned> getInstrShapeForEncoding(Attribute encoding) {
  assert(isSupportedStMatrixEncoding(encoding) &&
         "stmatrix layout requested for unsupported encoding");
#ifdef USE_MUSA
  if (auto mt = dyn_cast<MthreadsSqmmaEncodingAttr>(encoding))
    return SmallVector<unsigned>(mt.getInstrShape().begin(),
                                 mt.getInstrShape().end());
  auto mma = cast<NvidiaMmaEncodingAttr>(encoding);
  return SmallVector<unsigned>(mma.getInstrShape().begin(),
                               mma.getInstrShape().end());
#else
  if (auto mma = dyn_cast<NvidiaMmaEncodingAttr>(encoding))
    return SmallVector<unsigned>(mma.getInstrShape().begin(),
                                 mma.getInstrShape().end());
  auto mt = cast<MthreadsSqmmaEncodingAttr>(encoding);
  return SmallVector<unsigned>(mt.getInstrShape().begin(),
                               mt.getInstrShape().end());
#endif
}

static SmallVector<unsigned> getWarpsPerCTAForEncoding(Attribute encoding) {
  assert(isSupportedStMatrixEncoding(encoding) &&
         "stmatrix layout requested for unsupported encoding");
#ifdef USE_MUSA
  if (auto mt = dyn_cast<MthreadsSqmmaEncodingAttr>(encoding))
    return mt.getWarpsPerCTA();
  auto mma = cast<NvidiaMmaEncodingAttr>(encoding);
  return SmallVector<unsigned>(mma.getWarpsPerCTA().begin(),
                               mma.getWarpsPerCTA().end());
#else
  if (auto mma = dyn_cast<NvidiaMmaEncodingAttr>(encoding))
    return SmallVector<unsigned>(mma.getWarpsPerCTA().begin(),
                                 mma.getWarpsPerCTA().end());
  return cast<MthreadsSqmmaEncodingAttr>(encoding).getWarpsPerCTA();
#endif
}

static CTALayoutAttr getCTALayoutForEncoding(Attribute encoding) {
  assert(isSupportedStMatrixEncoding(encoding) &&
         "stmatrix layout requested for unsupported encoding");
#ifdef USE_MUSA
  if (auto mt = dyn_cast<MthreadsSqmmaEncodingAttr>(encoding))
    return mt.getCTALayout();
  return cast<NvidiaMmaEncodingAttr>(encoding).getCTALayout();
#else
  if (auto mma = dyn_cast<NvidiaMmaEncodingAttr>(encoding))
    return mma.getCTALayout();
  return cast<MthreadsSqmmaEncodingAttr>(encoding).getCTALayout();
#endif
}

static SmallVector<unsigned>
getTensorShapePerCTAForEncoding(Attribute encoding,
                                ArrayRef<int64_t> tensorShape) {
  assert(isSupportedStMatrixEncoding(encoding) &&
         "stmatrix layout requested for unsupported encoding");
  auto convertShape = [](ArrayRef<int64_t> shape64) {
  ...
  if (auto mma = dyn_cast<NvidiaMmaEncodingAttr>(encoding))
    return convertShape(getShapePerCTA(mma, tensorShape));
  auto mt = cast<MthreadsSqmmaEncodingAttr>(encoding);
  return convertShape(getShapePerCTA(mt, tensorShape));
}
...
bool canUseStMatrix(RankedTensorType tensorTy, ArrayRef<unsigned> repShape,
  ...
  if (!isSupportedStMatrixEncoding(encoding))
    return false;
  ...
  auto tensorShapePerCTA =
      getTensorShapePerCTAForEncoding(encoding, tensorTy.getShape());
...
std::optional<LinearLayout> chooseStMatrixLayoutLeadingOffset(
  ...
  // Expand the `warp` dimension according to warpsPerCTA.
  auto encoding = tensorTy.getEncoding();
  auto warpsPerCTA = getWarpsPerCTAForEncoding(encoding);
  layout *= identityND(kWarp, warpsPerCTA, /*order=*/{0, 1}, {kRow, kCol})
                .transposeOuts(llvm::to_vector(layout.getOutDimNames()));

  // Expand the `register` dimension so the size of columns matches `n`.
  int n = getInstrShapeForEncoding(encoding)[1];
  ...
  auto ret = combineCtaCgaWithShape(layout, getCTALayoutForEncoding(encoding),
                                    tensorTy.getShape());
